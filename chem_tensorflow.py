...
class ChemModel(object):
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                with tf.variable_scope("regression_gate"):
                    self.weights['regression_gate_task%i' % task_id] = MLP(2 * self.params['hidden_size'], 1, [self.params['hidden_size'], self.params['hidden_size'] // 2], self.placeholders['out_layer_dropout_keep_prob'])
                with tf.variable_scope("regression"):
                    self.weights['regression_transform_task%i' % task_id] = MLP(self.params['hidden_size'], 1, [self.params['hidden_size']//2, self.params['hidden_size']//4], self.placeholders['out_layer_dropout_keep_prob'])
                ### GATED ?? GCN
                # is this the last layer, is task the edge
                # v x 1
    ...
    #### depends on the child model - GRU or GCN
    ...
    def train(self):
        ...
        with self.graph.as_default():
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                ## train entire data here
                train_loss, train_accs, train_errs, train_speed = self.run_epoch("epoch %i (training)" % epoch, self.train_data, True)
                ...
                valid_loss, valid_accs, valid_errs, valid_speed = self.run_epoch("epoch %i (validation)" % epoch, self.valid_data, False)
